    ...
    if is_admin():
        ...
        if not safeFileList[0] == "未设置密码":
            if not md5(input("请输入密码验证身份：").encode(encoding='UTF-8')).hexdigest()==safeFileList[0]:
                print("密码错误！")
                sys.exit()
        print("当前webhook为:%s\n当前密码md5值为:%s"%(safeFileList[1],safeFileList[0]))
        if input("是否修改数据?(y/n):")=="y":
            ...
            if userinput=="密码":
                ...
                if not userinput==userinput2:
                    print("输入密码不一致，自动退出")
                    main(savefile)
                safeFileList[0]=md5(userinput.encode(encoding='UTF-8')).hexdigest()
...
def send_request(datas,ban=False):
    if (safeLoadConfigFile()[1][:5]=="https") and (not ban):
        ...
        f=requests.post(url=safeLoadConfigFile()[1],data=json.dumps(program),headers=headers)
