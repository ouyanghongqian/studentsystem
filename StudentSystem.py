...
import json
import urllib
safeText = ["用户尝试录入学生信息", "用户正在查找学生信息", "用户正在删除学生信息",
            "用户正在显示所有学生信息", "用户正在选择/从网络上导入文件", "用户正在修改学生信息", "用户正在修改安全模块配置"]
...
def ifHaveNewVer():
    try:
        ...
        internetCode = internetCode.text.split()[3]
...
def update():
    updateCode = requests.get(
        "https://gitee.com/oyhqnbnb/studentsystem/raw/main/StudentSystem.py").text
